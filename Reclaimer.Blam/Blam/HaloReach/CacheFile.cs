            ...
            using (var reader = CreateReader(MetadataTranslator))
            {
                ...
                switch (CacheType)
                {
                    ...
                    case CacheType.HaloReachRetail:
                        LocaleIndex = new LocaleIndex(this, 656, 68, 12);
                        break;
                }
            }
    ...
    public class TagIndex : ITagIndex<IndexItem>
    {
        ...
        internal void ReadItems()
        {
            ...
            using (var reader = cache.CreateReader(cache.MetadataTranslator))
            {
                ...
                var decrypted = reader.ReadAesBytes(cache.Header.FileTableSize, (cache.CacheType == CacheType.HaloReachBeta || cache.CacheType == CacheType.HaloReachAlpha) ? CacheFile.BetaKey : CacheFile.FileNamesKey);
    ...
    public class StringIndex : IStringIndex
    {
        ...
        internal void ReadItems()
        {
            using (var reader = cache.CreateReader(cache.HeaderTranslator))
            {
                ...
                var decrypted = reader.ReadAesBytes(cache.Header.StringTableSize, (cache.CacheType == CacheType.HaloReachBeta || cache.CacheType == CacheType.HaloReachAlpha) ? CacheFile.BetaKey : CacheFile.StringsKey);
                using (var ms = new MemoryStream(decrypted))
                using (var tempReader = new EndianReader(ms))
                {
